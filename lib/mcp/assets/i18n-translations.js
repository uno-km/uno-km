...
if (window.i18nManager) {
  ...
  "en": {
    "common": {
      "brand": "AMEVA-MCP-Hub",
      "releaseTag": "v3.0.0 (Universal Polyglot & AI Vector MCP Hub)",
      "pypiBtn": "PyPI (pip)",
      "npmBtn": "npm (Node.js)",
      "githubBtn": "GitHub",
    ...
    "home": {
      ...
      "challengeText": "Traditional MCP setups require gigabytes of compiler toolchains (JDK, Rust, Python virtualenvs) for each repository on the host machine with heavy memory overhead.",
      "breakthroughTitle": "The Architectural Breakthrough",
      "breakthroughText": "Executes WASI WebAssembly bytecodes in-memory inside a single Node process (<1ms execution) and enables live multi-repo subscription for AI tools.",
  ...
  "ko": {
    ...
    "home": {
      "title": "AMEVA-MCP-Hub",
      "subtitle": "무설치 다국어 WebAssembly 및 AI 벡터 지원 유니버설 MCP 허브 & SDK",
      "quickInstallTitle": "1줄 빠른 설치",
      "quickInstallDesc": "환경에 맞는 공식 패키지를 즉시 설치하세요:",
      "challengeTitle": "엔지니어링 도전 과제",
      "challengeText": "기존의 MCP 서버 구성은 리포지토리마다 호스트 PC에 JDK, Rust, Clang, Python 가상환경 등 수 기가바이트의 컴파일러를 요구하며, 다중 데몬 프로세스로 인한 높은 메모리 점유율을 유발합니다.",
      "breakthroughTitle": "아키텍처 혁신 및 해결책",
      "breakthroughText": "단일 Node.js 프로세스 내부에서 WASI WebAssembly 바이트코드를 인메모리로 실행하여 호스트 컴파일러 없이 C++, Rust, Java, Python, Go 도구를 <1ms 속도로 구동하며, 동적 깃허브 다중 리포지토리 구독과 실시간 AI 도구 전파를 지원합니다.",
