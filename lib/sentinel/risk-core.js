...
// ── v2 Semantic Separation: Actor Claim Derivation ─────────────────────
// Separates operator identity claim from automation risk score.
// NONE category maps to UNKNOWN (not BROWSER_USER) — absence of evidence
// is not evidence of humanity.
function deriveActorClaim(classification) {
  if (classification.category === "AI_AGENT") {
    return {
      type: "AI_OPERATOR",
      name: classification.claimedName || null,
      state: "CLAIMED",
      verification: "UNVERIFIED",
      basis: ["USER_AGENT_PATTERN"]
    };
  }
  if (classification.category === "SEARCH_ENGINE") {
    return {
      type: "SEARCH_BOT",
      name: classification.claimedName || null,
      state: "CLAIMED",
  ...
  // Default: no claim detected — NOT "BROWSER_USER"
  return {
    type: "UNKNOWN",
    name: null,
    state: "NONE",
...
// ── v2 Single-Signal Blocking Guard Rail ───────────────────────────────
// Prevents TEMPORARY_DENY from being issued based on passive fingerprint
// signals alone. Requires abuse evidence from multiple dimensions.
var PASSIVE_SIGNAL_CODES = new Set([
  "automation.webdriver",
  "automation.headless_deep",
  "header.http_missing_headers",
  "environment.touch_mismatch",
  "header.suspicious_ua"
]);
function canTemporarilyDeny(evidence) {
  var normalized = Array.isArray(evidence) ? evidence : [];
  if (normalized.length < 2) return false;
  var hasNonPassive = normalized.some(
    function(item) { return item && item.rule && !PASSIVE_SIGNAL_CODES.has(item.rule); }
  );
  return hasNonPassive && normalized.length >= 2;
}
...
function evaluateWithTrust(signals = {}, trustedState, optionsOrPolicy = defaultPolicy) {
  ...
  // v2 Guard Rail: prevent TEMPORARY_DENY from passive-only evidence
  if (recommendedScoreAction === "TEMPORARY_DENY" /* TEMPORARY_DENY */ && !canTemporarilyDeny(evidence)) {
    recommendedScoreAction = "RATE_LIMIT" /* RATE_LIMIT */;
  }
  ...
  // ── v2 Multi-Axis Assessment ───────────────────────────────────────
  const actorClaim = deriveActorClaim(classification);
  const riskLevel = finalScore < 40 ? "LOW_AUTOMATION_RISK"
    : finalScore < 70 ? "INDETERMINATE_AUTOMATION"
    : "HIGH_AUTOMATION_RISK";
  const normalizedEvidence = Array.isArray(evidence)
    ? evidence.map(function(item) {
        return { code: item && item.rule || null, weight: Number.isFinite(item && item.score) ? item.score : null };
...
`;
var DEFAULT_ECOSYSTEM_DOCS = {
  "/lib/stt": `
# \u{1F399}\uFE0F Termux-STT Technical Specification
- **PyPI / npm**: \`termux-stt\`
...
`,
  "/lib/train": `
# \u{1F682} Termux-Train Technical Specification
...
`,
  "/sentinel": `
# \u{1F6E1}\uFE0F AMEVA-Sentinel Technical Specification
...
`,
  "/lib/diffusion": `
# \u{1F3A8} Termux-Diffusion Technical Specification
...
`,
      ...
      estimatedHtmlBytes: config.estimatedHtmlBytes ?? 18e4
      // 180KB default HTML
    };
  }
  /**
...
`;
    ...
    const originalBytes = servedBytes;
    const savedBytes = 0;
   ...
   */
  static aggregateGeoAnalytics(logs) {
    ...
    const topAiPaths = Object.entries(pathCounts).map(([path, data]) => ({ path, count: data.count, savedBytes: data.savedBytes })).sort((a, b) => b.count - a.count).slice(0, 10);
    return {
      totalDeliveries,
      totalBytesServed,
      totalBytesSaved,
...
export {
  ...
  CompositeSink,
  DEFAULT_AUTHORITY_HEADER,
  DEFAULT_ECOSYSTEM_DOCS,
  GeoDeliveryEngine,
