...
var Sentinel = class {
  ...
  async collect(req) {
    ...
    const hasSecFetch = Boolean(getHeader("sec-fetch-dest") || getHeader("sec-fetch-mode") || getHeader("sec-fetch-site"));
    const hasAcceptLang = Boolean(getHeader("accept-language"));
    const isHttpMissingHeaders = isBrowserUA && (!hasSecFetch && !hasAcceptLang);
