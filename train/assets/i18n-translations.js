...
if (window.i18nManager) {
  ...
  "en": {
    ...
    "home": {
      "title": "Termux-Train",
      "subtitle": "Ultra-Lightweight On-Device Tensor & DAG Autograd Deep Learning Framework",
      ...
      "challengeText": "Training or fine-tuning neural networks on mobile devices with standard deep learning toolchains causes Out-Of-Memory (OOM) crashes and excessive disk paging.",
      "breakthroughTitle": "The Architectural Breakthrough",
      "breakthroughText": "Implements a pure C++/Python dynamic computation DAG with SafeTensors zero-copy serialization and memory-mapped LoRA backpropagation on ARM64.",
      ...
      "features": {
        "0": {
          "title": "Zero-Copy SafeTensors Serialization",
          "desc": "Loads and saves model checkpoints with zero memory duplication via mmap."
        },
        "1": {
          "title": "Dynamic DAG Autograd Engine",
          "desc": "Closed-form gradient computation with minimal heap allocation during backward pass."
        },
        "2": {
          "title": "On-Device LoRA Fine-Tuning",
          "desc": "Optimized low-rank adaptation matrix training directly on mobile CPU/NEON cores."
        }
      }
    }
  },
  "ko": {
    ...
    "home": {
      "title": "Termux-Train",
      "subtitle": "SafeTensors 제로 카피 및 LoRA 파인튜닝 지원 경량 온디바이스 DAG Autograd 딥러닝 프레임워크",
      "quickInstallTitle": "1줄 빠른 설치",
      "quickInstallDesc": "환경에 맞는 공식 패키지를 즉시 설치하세요:",
      "challengeTitle": "엔지니어링 도전 과제",
      "challengeText": "표준 딥러닝 툴체인으로 모바일에서 모델을 학습하거나 파인튜닝하면 막대한 프레임워크 오버헤드로 인해 메모리 부족(OOM) 오류가 발생합니다.",
      "breakthroughTitle": "아키텍처 혁신 및 해결책",
      "breakthroughText": "SafeTensors 제로 카피 직렬화와 메모리 맵 LoRA 역전파를 결합한 초경량 동적 계산 DAG 엔진으로 모바일 단말기 상에서 모델 파인튜닝을 실현합니다.",
      ...
      "features": {
        "0": {
          "title": "SafeTensors 제로 카피 메모리 매핑",
          "desc": "mmap 메모리 매핑을 통해 가중치 복사 없이 즉각 체크포인트를 로드하고 저장합니다."
        },
        "1": {
          "title": "동적 계산 그래프(DAG) Autograd 엔진",
          "desc": "역전파 수행 시 힙 할당을 최소화하여 닫힌 형태 수식으로 엄밀하게 그래디언트를 계산합니다."
        },
        "2": {
          "title": "온디바이스 LoRA 파인튜닝",
          "desc": "모바일 CPU 환경에서 저순위 어댑테이션(LoRA) 행렬을 직접 학습하고 가중치를 병합합니다."
        }
      }
    }
  },
