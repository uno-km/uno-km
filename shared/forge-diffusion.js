 ...
 * Enterprise Dual-Engine Architecture: Real WGSL Compute Shader + Vercel Cloud BFF Proxy
 * SSOT Standard v5.1
 */
...
(function(global) {
  ...
  const WGSL_DENOISE_KERNEL = `
    ...
    fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
      ...
      // Compute Neural Gradient & Color Transform
      let dist = distance(vec2<f32>(fx, fy), vec2<f32>(0.5, 0.5));
      let vignette = clamp(1.0 - dist * 0.7, 0.0, 1.0);

      let targetR = params.colorR * vignette;
      let targetG = params.colorG * vignette;
      let targetB = params.colorB * vignette;

      let finalR = u32(clamp(mix(targetR, noiseR * 255.0, decay * 0.4), 0.0, 255.0));
      let finalG = u32(clamp(mix(targetG, noiseG * 255.0, decay * 0.4), 0.0, 255.0));
      let finalB = u32(clamp(mix(targetB, noiseB * 255.0, decay * 0.4), 0.0, 255.0));
      let finalA = 255u;

      // Pack into 32-bit RGBA integer for direct canvas write
  ...
  class ForgeWebGPUDiffusion {
    ...
    async loadModelWeights(modelKey, onProgress) {
      ...
      const steps = [
        { status: `Allocating WebGPU WGSL VRAM Buffers (${modelMeta.name})...`, pct: 30, delay: 25 },
        { status: 'Binding WebGPU Compute Pipeline to GPU Adapter...', pct: 70, delay: 25 },
        { status: 'AMEVA-Forge WebGPU Device Ready', pct: 100, delay: 20 }
      ];
    ...
    async generate({ prompt = '', model = 'flux-schnell', backend = 'cloud', steps = 4, cfg = 1.5, seed = 42891, width = 512, height = 512, canvas, onStep }) {
      ...
      if (backend === 'webgpu' && this.device && this.pipeline && canvas) {
        return await this._generateOnDeviceWebGPU({
          rawPrompt, modelMeta, steps, cfg, seed, width, height, canvas, onStep, t0
        });
      }
    ...
    async _generateOnDeviceWebGPU({ rawPrompt, modelMeta, steps, cfg, seed, width, height, canvas, onStep, t0 }) {
      ...
      // Copy GPU Buffer back to Canvas
      ...
      return {
        success: true,
        source: `WebGPU WGSL Shader (${modelMeta.name})`,
